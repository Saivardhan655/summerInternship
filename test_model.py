...
def load_and_predict(features_df, model_path):
    model = load_model(model_path)
    feature_array = features_df[['avg_ear', 'avg_mar', 'avg_pitch', 'avg_roll', 'avg_yaw']].values
    
    # Reshape according to LSTM's expected input shape
    feature_array = feature_array.reshape((feature_array.shape[0], 1, feature_array.shape[1]))  # 1 time step
    predictions = model.predict(feature_array)
    return predictions

def process_videos(video_dir, output_dir, model_path):
    ...
    # After processing all videos, create DataFrame and predict
    if all_features:
        ...
        predictions = load_and_predict(features_df, model_path)
...
if __name__ == "__main__":
    ...
    model_path = "C:/Users/DELL/summerInternship/data/trained_models/best_model.keras"
    process_videos(video_dir, output_dir, model_path)