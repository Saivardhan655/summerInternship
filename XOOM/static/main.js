...
navigator.mediaDevices
    ...
    .then((stream) => {
        ...
        const frameInterval = setInterval(() => {
            const frameData = captureVideoFrame(myVideo);
        ...
        socket.on('attentiveness-update', (data) => {
            const videoElement = document.querySelector(`[data-userid="${data.userId}"]`);
            if (videoElement) {
                // Visual indication of attentiveness
                videoElement.style.opacity = data.attentiveness > 0.5 ? 1 : 0.5;
                
                // Optional: Add attentiveness score display
                const scoreDisplay = document.createElement('div');
                scoreDisplay.textContent = `Attentiveness: ${(data.attentiveness * 100).toFixed(2)}%`;
                videoElement.parentNode.appendChild(scoreDisplay);
            }
        });
...
socket.on("user-attentiveness", (data) => {
    // Update UI to show user's attentiveness
    const userElement = document.getElementById(data.userId);
    if (userElement) {
        // Visual indicator of attentiveness
        userElement.style.opacity = data.attentiveness > 0.5 ? 1 : 0.5;
    }
});
...
const addVideoStream = (videoEl, stream, name) => {
    ...
    videoEl.setAttribute('data-userid', peer.id);
